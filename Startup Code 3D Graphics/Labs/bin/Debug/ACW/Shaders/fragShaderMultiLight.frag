...
void main()
{
	...
	for(int i = 0; i < 3; ++i)
	{
		...
		FragColour +=  vec4(texture(uTextureSampler, oTexCoords)) * vec4(	uLight[i].AmbientLight * uMaterial.AmbientReflectivity + 
		...
		//FragColour +=  vec4(texture(uTextureSampler, oTexCoords));
